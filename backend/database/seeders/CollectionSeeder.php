...
class CollectionSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $names = [
            ...
            // Trending is REAL engagement data — top product-detail views in the
            // last 30 days (see product_views). Seeded with deterministic view
            // counts (see seedProductViews()) so a fresh install answers it;
            // live traffic reshapes it thereafter.
            ['name' => 'Trending', 'description' => 'What the store is loving right now.', 'image' => "{$imagesDir}/trending.png", 'query' => fn () => Product::where('is_active', true)
                ->whereHas('views', fn ($q) => $q->where('product_views.created_at', '>=', now()->subDays(30)))
                ->withCount(['views' => fn ($q) => $q->where('product_views.created_at', '>=', now()->subDays(30))])
    ...
    /**
     * Deterministic demo engagement for the Trending rail: products 5–14 each
     * get 20 views spread over the last 30 days, so the Trending query (top
     * by views, ties by product id) returns exactly ids 5–14 — distinct from
     * New Arrivals (latest ~ids 45–54) and Bestsellers (featured), with only
     * the acceptable overlap (6, 11 are featured too).
     */
    private function seedProductViews(): void
    {
        if (DB::table('product_views')->exists()) {
            return;
        }

        foreach (range(5, 14) as $productId) {
            foreach (range(1, 20) as $view) {
                DB::table('product_views')->insert([
                    'product_id' => $productId,
                    'created_at' => now()->subMinutes(random_int(5, 60 * 24 * 30)),
                ]);
            }
        }
    }
