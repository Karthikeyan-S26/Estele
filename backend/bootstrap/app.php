...
return Application::configure(basePath: dirname(__DIR__))
    ...
    ->withMiddleware(function (Middleware $middleware): void {
        // Mobile/API clients are cross-origin — CORS for /api/* only. The
        // storefront stays same-origin and is unaffected.
        $middleware->api(prepend: [
            \Illuminate\Http\Middleware\HandleCors::class,
        ]);
        // Mobile/jewellery.com API clients terminate TLS at their edge proxy and
        ...
        //
        // Only trust proxies on the private ranges the edge proxy actually sits on
        // (and loopback for local dev). Trusting '*' lets a remote client spoof
        // X-Forwarded-For and bypass the OTP/login throttles — the security
        // review flagged exactly that.
    ...
    ->withSchedule(function (\Illuminate\Console\Scheduling\Schedule $schedule) {
        // Old Jewellery bidding: close the 3h window the moment it passes.
        $schedule->command('sell:bids-close')->everyMinute()->withoutOverlapping();
        // Strike expired wallet credits off balances hourly.
