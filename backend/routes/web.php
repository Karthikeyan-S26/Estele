...
use App\Http\Controllers\SellVendorController;
...
/*
|--------------------------------------------------------------------------
| Old Jewellery — vendor buy-back (tokenized, no session auth)
|--------------------------------------------------------------------------
| Vendors reach these via an emailed invitation token. The token IS the
| authorization: it identifies the invitation AND (after acceptance) allows
| bidding on that request. admin.sell-requests sub-group lives in Filament.
*/

Route::prefix('sell/vendors')->name('sell.vendor.')->group(function () {
    Route::get('/invitations/{token}', [SellVendorController::class, 'invitation'])
        ->name('invitation')
        ->middleware('throttle:60,1');

    Route::post('/invitations/{token}/accept', [SellVendorController::class, 'accept'])
        ->name('accept')
        ->middleware('throttle:20,1');

    Route::post('/invitations/{token}/decline', [SellVendorController::class, 'decline'])
        ->name('decline')
        ->middleware('throttle:20,1');

    Route::post('/bids', [SellVendorController::class, 'storeBid'])
        ->name('bid')
        ->middleware('throttle:60,1');
}); 